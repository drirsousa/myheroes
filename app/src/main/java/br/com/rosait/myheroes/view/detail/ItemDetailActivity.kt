package br.com.rosait.myheroes.view.detail
...
import br.com.rosait.myheroes.databinding.ActivityItemDetailBinding
...
class ItemDetailActivity : BaseActivity() {
    ...
    private fun setupDetailItem() {
        mViewModel.mItem.observe(this, Observer { item ->
            setItemViews(item.id, item.name,"${item.thumbnail.path}.${item.thumbnail.extension}", item.description, false)
        })
    }

    private fun setupDetailFavoriteItem() {
        mViewModel.mItemFavorite.observe(this, Observer { itemFavorite ->
            setItemViews(itemFavorite.id, itemFavorite.name, itemFavorite.thumbnail, itemFavorite.description, true)
            mViewModel.setLoading(false)
        })
    }

    private fun setItemViews(id: Long, name: String?, imagePath: String, description: String?, isItemEntity: Boolean) {
        name?.let {
